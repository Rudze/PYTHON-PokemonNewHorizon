...
class Game:
    ...
    def _on_slot_swap(self, a: int, b: int) -> None:
        """Callback drag-and-drop items — annule tout refresh API en attente."""
        self.player.inv.swap_hud_slots(a, b)
        self._inv_pending_refresh = False   # évite que le reload écrase le swap local

    def _on_party_swap(self, a: int, b: int) -> None:
        """Callback drag-and-drop équipe — synchronise le nouvel ordre au serveur."""
