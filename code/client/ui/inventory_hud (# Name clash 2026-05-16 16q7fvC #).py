...
_ICONS_SHINY_DIR = SPRITES_DIR / "iconsshiny"
...
# Couleurs (adaptées au fond blanc style Animal Crossing)
C_SLOT         = (160, 160, 160)       # gris  — normal (visible sur blanc)
C_SLOT_SEL     = (255, 215, 50)        # doré  — sélectionné
C_SLOT_HOVER   = (80,  160, 255)       # bleu  — cible drag
C_SLOT_FILL    = (230, 230, 230, 140)  # fond légèrement gris
C_QTY          = (255, 255, 255)
C_NAME         = (255, 255, 255)
...
class InventoryHUD:
    ...
    def _poke_icon_frames(self, pkmn) -> list[pygame.Surface]:
        ...
        folder = _ICONS_SHINY_DIR if shiny else _ICONS_DIR
    ...
    def _draw_party(self, party: list) -> None:
        ...
        r_click = rbtn and not self._prev_rbtn
        self._prev_rbtn = rbtn

        for i in range(self._N_SLOTS):
            sx = W - MX - CARD_W   # bord droit
            ...
            # Bordure
            border_col = (120, 120, 120) if pkmn is None else (200, 80, 80) if is_ko else (200, 200, 200)
            pygame.draw.rect(disp, border_col, (sx, sy, CARD_W, CARD_H), 1, border_radius=6)

            if pkmn is None:
                continue

            # Icône (frame 2 quand le curseur survole le slot)
            frames = self._poke_icon_frames(pkmn)
            if frames:
                hovered = pygame.Rect(sx, sy, CARD_W, CARD_H).collidepoint(mouse)
                frame = frames[1 if hovered and len(frames) > 1 else 0]
            ...
            if r_click and slot_rect.collidepoint(mouse):
                self._ctx_pkmn = pkmn
                # Le slot est à droite → menu contextuel apparaît à gauche
                self._ctx_rect = pygame.Rect(sx - 94, sy, 90, 30)

        # ── Dessin du menu contextuel ──────────────────────────────────
