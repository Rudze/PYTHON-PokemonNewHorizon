...
class _PokemonStatsPage:
    ...
    def draw(
        ...
        bg.fill((12, 12, 24, 235))
        disp.blit(bg, (cx, cy))
        pygame.draw.rect(disp, (70, 70, 150), (cx, cy, cw, ch), 1, border_radius=6)
        ...
        for i, name in enumerate(self._TABS):
            ...
            ts.fill((50, 50, 120, 240) if active else (20, 20, 60, 180))
            disp.blit(ts, (tx, tabs_y))
            pygame.draw.rect(disp, (100, 100, 200) if active else (50, 50, 90),
                             (tx, tabs_y, tab_w, self._TAB_H), 1, border_radius=3)
            if self._f_tab:
                tt = self._f_tab.render(name, True,
                                        (230, 230, 255) if active else (130, 130, 180))
        ...
        if self._f_hdr:
            nt = self._f_hdr.render(name, True, (240, 240, 255))
            disp.blit(nt, (rx, ry));  ry += nt.get_height() + 3
            lt = self._f_hdr.render(f"Nv. {level}", True, (160, 220, 160))
    ...
    def _row(self, disp, x, y, w, label, value, col=(200, 220, 255)):
        h = 0
        if self._f_title:
            lt = self._f_title.render(label, True, (140, 140, 190))
    ...
    def _bar_row(self, disp, x, y, w, label, val, max_val, col_bar=(80,160,255)):
        ...
        bw = w;  bh = 9
        pygame.draw.rect(disp, (30, 30, 50), (x, y + h, bw, bh), border_radius=4)
    ...
    def _pg_ev(self, disp, x, y, w):
        ...
        for lbl, k in [("PV","hp"),("Attaque","atk"),("Defense","dfe"),
            ...
            y += self._bar_row(disp, x, y, w, f"{lbl}  {evs.get(k,0)}", evs.get(k,0), 252,
                               col_bar=(100,200,100))
        if self._f_title:
            tt = self._f_title.render(f"Total : {total} / 510", True, (220,200,100))
            disp.blit(tt, (x, y))
    ...
    def _pg_moves(self, disp, x, y, w, mouse_pos, full_disp, panel_cx, panel_cy):
        ...
        if ability and self._f_title:
            ab_lbl = self._f_title.render("Talent :", True, (140, 140, 190))
            ab_val = self._f_val.render(
                str(ability).replace("-", " ").replace("_", " ").title(),
                True, (220, 200, 100)) if self._f_val else None
            disp.blit(ab_lbl, (x, y))
            if ab_val:
                disp.blit(ab_val, (x + w - ab_val.get_width(), y))
            y += (ab_lbl.get_height() if ab_lbl else 18) + 6
            pygame.draw.line(disp, (50, 50, 90), (x, y), (x + w, y))
            y += 6
        ...
        for mv in moves:
            ...
            # Nom de l'attaque
            if self._f_title:
                nt = self._f_title.render(name[:20], True, (240, 240, 255))
            ...
            if self._f_small:
                ...
                ct = self._f_small.render(cat_fr, True, (200, 200, 140))
                disp.blit(ct, (x + 5 + tw2 + 6, y + mh - ct.get_height() - 7))

                # PP
                pp_t = self._f_small.render(f"PP {pp}/{mxpp}", True, (180, 220, 180))
    ...
    def _draw_move_tooltip(self, disp, mv, card_rect, panel_cx, panel_cy):
        ...
        lines = [
            ("", name_fr, (240, 240, 255)),
            ("Type",     _TYPE_FR.get(mtyp.lower(), mtyp.capitalize()), (200, 220, 255)),
            ("Categorie", _CAT_FR.get(cat, cat), (200, 200, 140)),
            ("Puissance", str(pwr),               (255, 200, 100)),
            ("Precision", f"{acc}%"  if acc != "-" else "-", (180, 220, 180)),
            ("PP",        f"{pp}/{mxpp}",          (180, 200, 240)),
            ("Priorite",  _priority_fr(prio),      (200, 160, 240)),
        ]
        if chance:
            lines.append(("Chance", f"{chance}%", (240, 180, 120)))
        if effect:
            lines.append(("", effect, (210, 210, 180)))
        ...
        bg.fill((12, 12, 28, 240))
        disp.blit(bg, (tx, ty))
        pygame.draw.rect(disp, (*tc, 200), (tx, ty, tw, th), 1, border_radius=6)
