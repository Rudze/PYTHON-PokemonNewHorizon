"""
Renomme les icônes Pokémon du dossier assets/sprite/Icons
au format : {numéro_pokédex}-b-s.png  (variante : -b-s-1.png, -b-s-2.png …)

Lancer avec --dry-run pour prévisualiser sans modifier les fichiers.
"""
import os
import re
import sys

ICONS_DIR = os.path.join(os.path.dirname(__file__), "assets", "sprite", "iconsshiny")

# ──────────────────────────────────────────────────────────────────────────────
# Correspondance nom-fichier → numéro Pokédex
# Les noms sont en MAJUSCULES, sans apostrophes ni tirets, comme dans les fichiers.
# ──────────────────────────────────────────────────────────────────────────────
POKEDEX = {
    # ── Gen 1 ──────────────────────────────────────────────────────────────
    ...
    "COSMOG": 789, "COSMOEM": 790,
    "SOLGALEO": 791, "LUNALA": 792,
    "NIHILEGO": 793, "BUZZWOLE": 794, "PHEROMOSA": 795,
    "XURKITREE": 796, "CELESTEELA": 797, "KARTANA": 798,
    "GUZZLORD": 799,
    "NECROZMA": 800,
    "MAGEARNA": 801,
    "MARSHADOW": 802,
    ...
    "DRACOZOLT": 880, "ARCTOZOLT": 881,
    "DRACOVISH": 882, "ARCTOVISH": 883,
    "DURALUDON": 884,
    ...
    "DUDUNSPARCE": 982,
    "KINGAMBIT": 983,
    "GREATTUSK": 984, "SCREAMTAIL": 985,
    "BRUTEBONNET": 986, "FLUTTERMANE": 987,
    "SLITHERWING": 988, "SANDYSHOCKS": 989,
    "IRONTREADS": 990, "IRONBUNDLE": 991,
    "IRONHANDS": 992, "IRONJUGULIS": 993,
    "IRONMOTH": 994, "IRONTHORNS": 995,
    "FRIGIBAX": 996, "ARCTIBAX": 997, "BAXCALIBUR": 998,
    "GIMMIGHOUL": 999, "GHOLDENGO": 1000,
    "WOCHIEN": 1001, "CHIENPAO": 1002,
    "TINGLU": 1003, "CHIYU": 1004,
    "ROARINGMOON": 1005, "IRONVALIANT": 1006,
    "KORAIDON": 1007, "MIRAIDON": 1008,
    "WALKINGWAKE": 1009, "IRONLEAVES": 1010,
    "DIPPLIN": 1011,
    "POLTCHAGEIST": 1012, "SINISTCHA": 1013,
    "OKIDOGI": 1014, "MUNKIDORI": 1015, "FEZANDIPITI": 1016,
    "OGERPON": 1017,
    "ARCHALUDON": 1018, "HYDRAPPLE": 1019,
...
def parse_filename(filename: str) -> tuple[str, int | None, bool]:
    """
    Décompose un nom de fichier PNG.
    Retourne (base_name, variant_number_or_None, is_female).
    Exemples :
      'ABSOL.png'        → ('ABSOL', None, False)
      'ABSOL_1.png'      → ('ABSOL', 1, False)
      'PIKACHU_female.png' → ('PIKACHU', None, True)
      'GYARADOS_female.png' → ('GYARADOS', None, True)
      'ELECTABUZZ_shadow.png' → ('ELECTABUZZ', 'shadow', False)
    """
    name = filename[:-4]  # enlève .png
    ...
    if m:
        ...
        name = name[: m.start()]
    elif name.endswith("_shadow"):
        shadow = True
        name = name[:-7]

    return name, variant, is_female, shadow  # type: ignore[return-value]


def build_rename_plan(folder: str) -> list[tuple[str, str]]:
    """
    Construit la liste (ancien_nom, nouveau_nom) pour tous les PNG du dossier.
    """
    files = sorted(f for f in os.listdir(folder) if f.endswith(".png"))

    # Grouper par base_name pour calculer les indices female
    from collections import defaultdict
    groups: dict[str, list] = defaultdict(list)
    for f in files:
        base, variant, is_female, shadow = parse_filename(f)
        groups[base].append((f, variant, is_female, shadow))

    plan = []
    skipped = []

    for base, entries in sorted(groups.items()):
        # 000.png = placeholder, on ignore
        if base == "000":
            skipped.append(("000.png", "placeholder ignoré"))
            continue

        dex_num = POKEDEX.get(base)
        if dex_num is None:
            for f, *_ in entries:
                skipped.append((f, f"nom '{base}' inconnu dans le Pokédex"))
            continue

        # Trouver le numéro de variante max pour placer les formes female après
        numeric_variants = [v for _, v, _, _ in entries if v is not None]
        max_variant = max(numeric_variants) if numeric_variants else 0

        female_counter = max_variant + 1
        shadow_counter = female_counter  # sera ajusté si female existe

        for f, variant, is_female, shadow in sorted(
            entries,
            key=lambda x: (
                x[1] is not None,   # numériques d'abord
                x[1] if x[1] is not None else 999,
                x[2],               # female
                x[3],               # shadow
            ),
        ):
            if variant is None and not is_female and not shadow:
                new_name = f"{dex_num}-b-s.png"
            elif variant is not None:
                new_name = f"{dex_num}-b-s-{variant}.png"
            elif is_female:
                new_name = f"{dex_num}-b-s-{female_counter}.png"
                female_counter += 1
                shadow_counter = female_counter
            elif shadow:
                new_name = f"{dex_num}-b-s-{shadow_counter}.png"
                shadow_counter += 1
...
def main():
    ...
    plan, skipped = build_rename_plan(ICONS_DIR)

    # Détecter les conflits (deux anciens fichiers → même nouveau nom)
    new_names = [new for _, new in plan]
    conflicts = {n for n in new_names if new_names.count(n) > 1}

    print(f"\n{'[DRY-RUN] ' if dry_run else ''}Renommage de {len(plan)} fichiers\n")

    errors = 0
    for old, new in plan:
        if new in conflicts:
            print(f"  ! CONFLIT  {old:50s} → {new}")
            errors += 1
            continue
        if old == new:
            continue  # déjà bon nom
        if dry_run:
            print(f"  {old:50s} -> {new}")
        else:
            src = os.path.join(ICONS_DIR, old)
            dst = os.path.join(ICONS_DIR, new)
            if os.path.exists(dst):
                print(f"  ! EXISTE DÉJÀ  {dst}")
                errors += 1
            else:
                os.rename(src, dst)

    if skipped:
        print(f"\nFichiers ignorés / non trouvés dans le Pokédex ({len(skipped)}) :")
        for f, reason in skipped:
            print(f"  {f:50s} — {reason}")

    if errors:
        print(f"\n{errors} erreur(s) — aucun fichier conflictuel n'a été renommé.")
    elif not dry_run:
        print("\nTerminé !")
    else:
        print("\n(Aucune modification — relancez sans --dry-run pour appliquer)")
